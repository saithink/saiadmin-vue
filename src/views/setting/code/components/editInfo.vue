      ...
      <a-form :model="form" ref="formRef">
        <a-tabs v-model:active-key="activeTab">
          <a-tab-pane title="配置信息" key="base_config">
            ...
            <a-row :gutter="24">
              ...
              <a-col :xs="24" :md="8" :xl="8">
                <a-form-item
                  ...
                  label-col-flex="auto"
                  :label-col-style="{ width: '100px' }"
                  extra="生成到模块仅调试模式有效，自动生成到前后端目录，自动处理菜单生成"
                >
            ...
            <a-row :gutter="24">
              <a-col :xs="24" :md="12" :xl="12">
                ...
                >
                  <a-radio-group
                    v-model:model-value="form.component_type"
                    type="button"
                  >
            ...
            <div v-if="form.component_type === 3">
              ...
              <a-row :gutter="24">
                <a-col :xs="24" :md="8" :xl="8">
                  ...
                  >
                    <a-input
                      v-model="formOptions.tag_id"
                      placeholder="请输入标签页ID"
                    ></a-input>
                  </a-form-item>
                </a-col>
                <a-col :xs="24" :md="8" :xl="8">
              ...
              <template #columns>
                <a-table-column dataIndex="sort" title="排序" :width="100">
                  <template #cell="{ rowIndex }"
                    ><a-input-number v-model="form.columns[rowIndex].sort"
                  /></template>
                </a-table-column>
                <a-table-column
                ...
                >
                  <template #cell="{ rowIndex }"
                    ><a-input
                      v-model="form.columns[rowIndex].column_comment"
                      allow-clear
                  /></template>
                </a-table-column>
                <a-table-column
                ...
                >
                  ...
                  <template #cell="{ rowIndex }"
                    ><a-checkbox v-model="form.columns[rowIndex].is_required"
                  /></template>
                </a-table-column>
                <a-table-column dataIndex="is_insert" title="插入" :width="80">
                  ...
                  <template #cell="{ rowIndex }"
                    ><a-checkbox v-model="form.columns[rowIndex].is_insert"
                  /></template>
                </a-table-column>
                <a-table-column dataIndex="is_edit" title="编辑" :width="80">
                  <template #title
                    >编辑
                    <a-tooltip content="全选 / 全不选" position="bottom"
                      ><a-checkbox @change="handlerAll($event, 'edit')"
                    /></a-tooltip>
                  </template>
                  <template #cell="{ rowIndex }"
                    ><a-checkbox v-model="form.columns[rowIndex].is_edit"
                  /></template>
                </a-table-column>
                <a-table-column dataIndex="is_list" title="列表" :width="80">
                  ...
                  <template #cell="{ rowIndex }"
                    ><a-checkbox v-model="form.columns[rowIndex].is_list"
                  /></template>
                </a-table-column>
                <a-table-column dataIndex="is_query" title="查询" :width="80">
                  ...
                  <template #cell="{ rowIndex }"
                    ><a-checkbox v-model="form.columns[rowIndex].is_query"
                  /></template>
                </a-table-column>
                <a-table-column dataIndex="is_sort" title="排序" :width="80">
                  ...
                  <template #cell="{ rowIndex }"
                    ><a-checkbox v-model="form.columns[rowIndex].is_sort"
                  /></template>
                </a-table-column>
                <a-table-column
                ...
                >
                  <template #cell="{ record, rowIndex }">
                    <a-space>
                      ...
                      <a-link
                        v-if="
                          !notNeedSettingComponents.includes(record.view_type)
                        "
                ...
                >
                  <template #cell="{ record, rowIndex }">
                    <a-select
                      ...
                      placeholder="选择数据字典"
                      :disabled="
                        !['select', 'radio', 'checkbox', 'transfer'].includes(
                          record.view_type
                        )
                      "
                    ></a-select>
          ...
          <a-tab-pane title="菜单配置" key="menu_config">
            <a-alert title="提示"
              >sql语句会生成所有接口。注意：前端页面根据选择进行按钮生成</a-alert
            >
            <a-checkbox-group
              direction="vertical"
              v-model="form.generate_menus"
              class="mt-3"
          ...
          <a-tab-pane title="关联配置" key="relation_config">
            <a-alert title="提示"
              >模型关联支持：一对一、一对多、一对一（反向）、多对多。</a-alert
            >
            <a-button @click="addRelation" type="primary" class="mt-3"
              ><icon-plus /> 新增关联</a-button
            >
            <div v-for="(item, index) in formOptions.relations" :key="index">
              <a-divider orientation="left">
                {{ item.name ? item.name : "定义新关联" }}
                <a-link @click="delRelation(index)" class="ml-5"
                  ><icon-delete /> 删除定义</a-link
                >
              </a-divider>
              <a-row :gutter="24">
                <a-col :xs="24" :md="12" :xl="12">
              ...
              <a-row :gutter="24">
                <a-col
                  ...
                  :md="12"
                  :xl="12"
                  v-show="item.type === 'belongsToMany'"
                >
                  <a-form-item
                    label="中间模型"
                    field="model"
...
const init = () => {
  ...
  if (record.value.component_type === 3) {
    ...
    formOptions.value.tag_view_name =
      record.value?.options?.tag_view_name ?? undefined;
  }
  ...
  // 请求所有字典类型
  dictType
    .getPageList({ saiType: "all" })
    .then((res) => (dicts.value = res.data));
};
