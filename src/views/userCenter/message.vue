  ...
  <div class="ma-content-block p-3 lg:h-full block lg:border-0 lg:flex justify-between">
    <ul class="w-full lg:w-52 msg-menu p-2 shadow" ref="msgMenuRef">
      <li
        ...
        :key="item"
        @click="getMessageList(item.value, index)"
      >
        <Component :is="typeIcon[item.value] ? typeIcon[item.value] : 'icon-message'" /> 
...
<script setup>
  ...
  onMounted(async () => {
    ...
    msgType.value.unshift(...[
      { value: 'receive_box', label: '收件箱' }, { value: 'send_box', label: '已发送' }
    ])
