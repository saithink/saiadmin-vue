	...
	<div class="sys-search-container">
		<div class="ssc-bg">
			<div class="w-6/12 mx-auto center-box">
				...
				<div class="mt-10">
					<a-input size="large" ref="searchInputRef" placeholder="搜索页面，支持名称、标识以及URL的模糊查询" @input="searchPage">
				...
				<div class="mt-5">
					<a-space size="large" class="flex justify-center">
						...
						<a-space
							><a-tag><icon-caret-up /></a-tag><a-tag><icon-caret-down /></a-tag><a-tag>切换搜索结果</a-tag></a-space
						>
				...
				<ul class="mt-10 results shadow-lg customer-scrollbar">
					<template v-for="res in resultList">
						<li class="flex items-center" v-if="res && res.path.indexOf(':') === -1 && res.components && res?.meta?.type === 'M'" @click="gotoPage(res)">
