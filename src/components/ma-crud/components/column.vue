<template>
	<template v-for="row in props.columns" :key="row[options.pk]">
		<template v-if="isFunction(row.hide) ? row.hide() : !row.hide">
			...
			>
				<column @refresh="() => refresh()" :isRecovery="props.isRecovery" :crudFormRef="props.crudFormRef" :columns="row.children">
					<template v-for="(childRow, childIndex) in row.children" :key="childIndex" #[childRow.dataIndex]="{ record, column, rowIndex }">
				...
				<template #cell="{ record, column, rowIndex }">
                  ...
                  ><icon-eye /> {{ options.see.text || '查看' }}</a-link> -->
					...
					<template v-else-if="row.customRender">
						<custom-render :column="column" :record="record" :render="row.customRender" :rowIndex="rowIndex"></custom-render>
					</template>
