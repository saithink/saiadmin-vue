...
const submit = async () => {
  ...
  if (currentAction.value === "add") {
    ...
    isFunction(options.afterAdd) &&
      (await options.afterAdd(response, formData));
  } else {
    if (
      isFunction(options.beforeEdit) &&
      !(await options.beforeEdit(formData))
    ) {
      return false;
    }
    response = await options.edit.api(formData[options.pk], formData);
    isFunction(options.afterEdit) &&
      (await options.afterEdit(response, formData));
  }
  if (response.code === 200) {
    ...
    (typeof response.code === "undefined" || response.code !== 200)
  ) {
    Message.clear();
    Message.error(
      response.message || response.msg || `${actionTitle.value}失败！`
    );
    return false;
  }
};
const open = () => {
  ...
  if (options.formOption.viewType === "tag" && currentAction.value !== "see") {
    ...
    queryParams.key =
      form.value[options.formOption?.titleDataIndex ?? ""] ??
      form.value[options.pk];
    ...
    if (currentAction.value === "add") {
      formStore.formList[options.formOption.tagId].addData = cloneDeep(
        form.value
      );
    } else {
      formStore.formList[options.formOption.tagId].editData[queryParams.key] =
        cloneDeep(form.value);
    }
    form.value = {};
    router.push(
      `/openForm/${options.formOption.tagId}` +
        tool.httpBuild(queryParams, true)
    );
  } else {
    componentName.value =
      options.formOption.viewType === "drawer" ? "a-drawer" : "a-modal";
...
const columnItemHandle = async (item) => {
  ...
  if (excludeColumns.includes(item.dataIndex)) {
    layoutColumns.value.set(item.dataIndex, {
      dataIndex: item.dataIndex,
      layoutFormRemove: true,
    });
    return;
  }
