  ...
  <a-layout-content class="flex flex-col lg:h-full relative w-full">
    <a-card :bordered="false">
      ...
      <div class="_crud-content">
        <a-row style="margin-bottom: 15px" v-if="!options.pageSimple">
          <a-col :span="16">
            <a-space>
                ...
                <template v-for="(row, index) in columns" :key="index">
                  <a-table-column
                    ...
                    <template #cell="{ record, column, rowIndex }">
                      ...
                      <slot v-else-if="row.dict" :name="row.dataIndex" v-bind="{ record, column, rowIndex }">
                        <a-tag :color="tool.getColor(record[row.dataIndex], dictList[row.dict] ?? [], row.colors || [])">{{
                          tool.getLabel(record[row.dataIndex], dictList[row.dict] ?? [])
                        }}</a-tag>
                      </slot>
                      <slot v-else-if="row.options" :name="row.dataIndex" v-bind="{ record, column, rowIndex }">
                        <a-tag :color="tool.getColor(record[row.dataIndex], row.options ?? [], row.colors || [])">{{
                          tool.getLabel(record[row.dataIndex], row.options ?? [])
                        }}</a-tag>
                      </slot>
...
const init = async () => {
  ...
  // 收集数据
  if (columns.value.length > 0 && columns.value[columns.value.length - 1].dataIndex !== '__operation' && options.value.operationColumn) {
    columns.value?.push({
  ...
  currentApi.value =
    isRecovery.value && options.value.recycleApi && isFunction(options.value.recycleApi) ? options.value.recycleApi : options.value.api
}
